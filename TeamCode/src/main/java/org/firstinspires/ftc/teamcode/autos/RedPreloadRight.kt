...
class RedPreloadRight : LinearOpMode() {
    override fun runOpMode() {
        ...
        robot.pose = Pose(2050, 3358, 0)

        val centerSpike = Pose(2127, 2859, 0)

        val spikePositions = hashMapOf(
            "left" to Pose(2360, 2624, -30),
            "middle" to Pose(2110, 2521, 0),
            "right" to Pose(1880, 2694, 30)
        )

        val backdropPositions = hashMapOf(
            "left" to Pose(980, 2650, 95),
            "middle" to Pose(980, 2759, 95),
            "right" to Pose(980, 2950, 95)
        )

        val preTurn = Pose(2110, 3054, 0)
        val turn = Pose(2110, 3054, 90)
        val park = Pose(1050, 3440, 0)
        ...
        fun deposit(): Command {
            return CommandGroup()
                .add(Command({ slides.height = 1000.0 }) {slides.state != Slide.SlideState.Idle})
                .add(timedCommand({ arm.up() }, Time(1.0)))
                .add(Command { output.open() })
                .add(timedCommand({}, Time(1.0)))
                .add(Command({ slides.height = 2000.0 }) {slides.state == Slide.SlideState.Idle})
                .add(timedCommand({}, Time(1.0)))
                .build()
        }
